using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class SlimeEffect : MonoBehaviour, IPoolable
{
    private Animator anim;
    private AnimationHash<SlimeEffectAniType> aniHash;
    public PoolItem PoolItemPre { get; set; }

    private void Awake()
    {
        anim = GetComponent<Animator>();
        aniHash = new AnimationHash<SlimeEffectAniType>(anim);
    }

    public void PlayEffect(SlimeEffectAniType type)
    {
        aniHash.PlayFirstFrame(type);
        StopAllCoroutines();
        StartCoroutine(DespawnCo());
    }

    public void PlayEffectLoop(SlimeEffectAniType type)
    {
        aniHash.PlayFirstFrame(type);
    }

    public void StopEffect()
    {
        StopAllCoroutines();
        StartCoroutine(DespawnCo());
    }

    private IEnumerator DespawnCo()
    {
        yield return null;

        float dis = anim.GetCurrentAnimatorStateInfo(0).length;
        yield return new WaitForSecondsRealtime(dis);

        PoolItemPre.Despawn();
    }

    public void OnSpawn() { }
    public void OnDespawn() => StopAllCoroutines();
}
